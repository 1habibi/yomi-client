...
export function UserAnimeLists({
  userId,
  readonly = false,
}: UserAnimeListsProps) {
  ...
  >("custom");
  const { data, isLoading } = useUserLists(userId, sortMode);
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-semibold">
          {readonly ? "Списки аниме" : "Мои списки"}
        </h2>
        {!readonly && sortMode === "custom" && (
          <select
            value={sortMode}
            onChange={(e) =>
              setSortMode(
        ...
        {readonly && (
          <select
            value={sortMode}
            onChange={(e) =>
              setSortMode(e.target.value as "date" | "rating" | "title")
            }
            className="border-input bg-background rounded-md border px-3 py-1.5 text-sm"
          >
            <option value="date">По дате</option>
